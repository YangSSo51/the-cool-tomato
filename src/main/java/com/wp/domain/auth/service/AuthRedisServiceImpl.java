...
public class AuthRedisServiceImpl implements AuthRedisService{
    ...
    @Override
    public boolean registRefreshToken(String email, String refreshToken) {
        //이미 RT가 등록되어 있으면 false
        if(jwtTokenRepository.findById(email).orElse(null) != null)return false;
        ...
        //RT 등록 성공 여부
        if(result.getId().equals(email) && result.getRefreshToken().equals(refreshToken)){
            return true;
        }
        else{
            return false;
    ...
    public JwtToken getRefreshToken(String email) {
        ...
        //조회 실패시 null 반환
        return jwtTokenRepository.findById(email).orElse(null);
    }

    @Override
    public boolean updateRefreshToken(String email, String refreshToken) {
        try {
    ...
    public boolean removeRefreshToken(String email) {
        ...
        try {
            ...
            jwtToken = jwtTokenRepository.findById(email).orElseThrow();
        }catch (NoSuchElementException e){
            //삭제 대상이 없어도 목적 달성
            return true;
        }
        ...
        //삭제 성공 여부
        try{
            jwtTokenRepository.findById(email).orElseThrow();
            return false;
        }catch (NoSuchElementException e){
            return true;
        }
