...
public class BlockManageController {
    ...
    public ResponseEntity<SuccessResponse<?>> getBlockManagesCache(@NotNull(message = "판매자 회원 ID를 입력해 주세요.") @PathVariable(value = "seller-id") Long sellerId) {
        GetBlockManageListResponse getBlockManageListResponse = blockManageService.getBlockManagesCache(sellerId);
    ...
    @PostMapping("/add")
    public ResponseEntity<SuccessResponse<?>> addBlocked(@Valid @RequestBody BlockedIdRequest blockListRequest) {
    ...
    public ResponseEntity<SuccessResponse<?>> removeBlocked(@Valid @RequestBody BlockedIdRequest blockListRequest) {
